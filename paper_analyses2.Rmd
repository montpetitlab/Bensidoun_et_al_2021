...
plt4_length <- ggtexttable(all_wilcox_length2, rows = NULL, theme = ttheme(base_size = 8))
  
ggarrange(ggarrange(plt2_length, plt3_length, plt1_length, legend = "bottom", common.legend = T, nrow = 1),
          plt4_length, nrow = 2)
```
...
```{r EXPRESSION_by_shared_set}
# sets -- anything that's more likely to be in basketless vs basket only
...
plt3_exp_scatter <- res_basketless_features %>%
...
plt4_exp_scatter <- res_basket_features %>%
  filter(gene %in% c(basket_nsb_up$gene, basket_nsb_down$gene)) %>%
  group_by(introns, change) %>% 
  summarise(mean_polya_vst = round(mean(mean_polya_vst), digits = 2),
            mean_polya_counts = round(mean(mean_polya), digits = 2)) %>%
...
ggarrange(plt2_exp_scatter, plt1_exp_scatter, plt3_exp_scatter,  plt4_exp_scatter,
...
plt1_polya <- ggplot(res_basketless_features %>%
              ...
              aes(x = polya_length)) +
  geom_density(alpha = .5, aes(fill = change)) +
  geom_density(data = features %>% filter(polya_length > 1), aes(x = polya_length), linetype = "dashed", color = "grey") +
  theme_minimal() +
  theme(plot.title = element_text(size=9),
        plot.title.position = "plot") +
  ggtitle(paste0("Basket, Basketless, All (n = ",
...
ggarrange(ggarrange(plt2_polya, plt3_polya, plt1_polya, legend = "bottom", common.legend = T, nrow = 1),
          plt4_polya, nrow = 2)
